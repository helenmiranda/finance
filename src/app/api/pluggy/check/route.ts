...
type ItemStatus = { status?: string; executionStatus?: string; lastUpdatedAt?: string; error?: { code?: string } | null };
...
export async function GET() {
  ...
  for (const connection of connections ?? []) {
    try {
      ...
      await supabase.from("pluggy_items").update({ status: item.status ?? "UPDATING", execution_status: item.executionStatus ?? null, error_code: item.error?.code ?? null }).eq("id", connection.id);
