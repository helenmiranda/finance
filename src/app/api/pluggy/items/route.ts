...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error } = await supabase.from("pluggy_items").upsert({ household_id: membership.household_id, connected_by: user.id, pluggy_item_id: item.id, connector_id: item.connector?.id ?? null, connector_name: item.connector?.name ?? "Instituição financeira", status: item.status ?? "UPDATING", execution_status: item.executionStatus ?? null, error_code: item.error?.code ?? null, provider_updated_at: item.lastUpdatedAt ?? null, status_detail: item.statusDetail ?? null, last_synced_at: item.lastUpdatedAt ?? null, updated_at: new Date().toISOString() }, { onConflict: "pluggy_item_id" });
    if (error) {
