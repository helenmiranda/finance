...
import { addAccount, updateFinancialNickname } from "../finance-actions";
    ...
    <DashboardShell active="accounts">
      <section className="content settings-content">
        ...
        <div className="settings-grid">
          ...
          <section className="items-column"><div className="section-heading"><h2>Contas cadastradas</h2><span className="count-badge">{accounts?.length ?? 0}</span></div>{!accounts?.length && <article className="card empty-state"><span>＋</span><h2>Nenhuma conta ainda</h2><p className="muted">Adicione a primeira conta para começar.</p></article>}{accounts?.map((account) => <article className="card account-item account-with-nickname" key={account.id}><span className="item-color" style={{ background: account.color ?? "#9fe870" }} /><div><strong>{account.nickname || account.name}</strong><small>{account.nickname ? `${account.name} · ` : ""}{account.institution_name || accountTypes[account.type]}</small></div><div className="item-value"><small>{account.current_balance_cents == null ? "Saldo inicial" : "Saldo atual"}</small><strong>{money.format((account.current_balance_cents ?? account.initial_balance_cents) / 100)}</strong></div><details className="nickname-editor"><summary>Editar apelido</summary><form action={updateFinancialNickname}><input type="hidden" name="kind" value="account" /><input type="hidden" name="id" value={account.id} /><input name="nickname" defaultValue={account.nickname ?? ""} maxLength={60} placeholder="Ex.: Nubank Helen" /><button type="submit">Salvar</button></form></details></article>)}</section>
        </div>
      </section>
    </DashboardShell>
  );
