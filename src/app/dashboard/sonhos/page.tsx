    ...
    <section className="dream-gallery">
      {dreams?.map((dream) => {
        ...
        return <article className={`card dream-card ${dream.status}`} key={dream.id} style={{ "--dream-color": dream.color } as React.CSSProperties}>
          {covers.get(dream.id) && <div className="dream-cover" role="img" aria-label={`Capa de ${dream.title}`} style={{ backgroundImage: `linear-gradient(180deg, transparent 45%, rgba(9, 29, 4, .6)), url("${covers.get(dream.id)}")` }}><span>{dream.emoji}</span></div>}
          ...
          {dream.status === "active" ? <details className="dream-contribution"><summary>+ Guardar para este sonho</summary><form action={addDreamContribution}><input type="hidden" name="dream_id" value={dream.id} /><label>Valor<input name="amount" inputMode="decimal" placeholder="100,00" required /></label><label>Data<input name="contributed_on" type="date" defaultValue={new Date().toISOString().slice(0, 10)} required /></label><label>Uma lembrança<input name="note" placeholder="Opcional" /></label><button type="submit">Guardar</button></form></details> : dream.status === "achieved" ? <div className="dream-celebration">Conquista desbloqueada 🏆</div> : <div className="dream-paused">Sonho pausado — o progresso continua guardado.</div>}
          ...
          <details className="dream-manage"><summary>Gerenciar sonho</summary><form action={uploadDreamCover} className="dream-cover-form"><input type="hidden" name="dream_id" value={dream.id} /><label>Capa do sonho<input name="cover" type="file" accept="image/jpeg,image/png,image/webp" required /></label><small>JPG, PNG ou WebP de até 5 MB.</small><button type="submit">{dream.cover_path ? "Trocar capa" : "Adicionar capa"}</button></form>{dream.cover_path && <form action={removeDreamCover} className="dream-remove-cover"><input type="hidden" name="dream_id" value={dream.id} /><button className="quiet-danger" type="submit">Remover capa atual</button></form>}<form action={updateDream}><input type="hidden" name="dream_id" value={dream.id} /><div className="dream-icon-picker"><label>Símbolo<input name="emoji" defaultValue={dream.emoji} maxLength={12} /></label><label>Cor<input className="color-input" name="color" type="color" defaultValue={dream.color} /></label></div><label>Nome<input name="title" defaultValue={dream.title} maxLength={80} required /></label><label>Por que importa?<textarea name="why_text" defaultValue={dream.why_text} maxLength={280} rows={3} required /></label><div className="dream-form-row"><label>Valor desejado<input name="target" inputMode="decimal" defaultValue={(dream.target_cents / 100).toFixed(2).replace(".", ",")} required /></label><label>Prazo<input name="target_date" type="date" defaultValue={dream.target_date ?? ""} /></label></div><button type="submit">Salvar mudanças</button></form>{dream.status !== "achieved" && <form action={toggleDreamStatus} className="dream-status"><input type="hidden" name="dream_id" value={dream.id} /><input type="hidden" name="status" value={dream.status === "paused" ? "active" : "paused"} /><button className="secondary-button" type="submit">{dream.status === "paused" ? "Retomar sonho" : "Pausar sonho"}</button></form>}<form action={deleteDream} className="dream-delete"><input type="hidden" name="dream_id" value={dream.id} /><label>Para remover definitivamente, digite REMOVER<input name="confirmation" autoComplete="off" required /></label><button className="quiet-danger" type="submit">Remover sonho e aportes</button></form></details>
        </article>;
      })}
    </section>
